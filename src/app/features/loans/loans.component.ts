...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, FormsModule, LoanFormComponent, AlertContainerComponent],
  template: `
    <div class="container-fluid p-0">
      <div class="row m-0">
        <div class="col-12 px-3 py-3">
                ...
                <div class="col-md-5 col-lg-4 d-flex justify-content-md-end align-items-center flex-wrap gap-2">
                  <div class="form-check form-switch me-3">
                    <input 
                      ...
                      (change)="applyFilters()"
                    >
                    <label class="form-check-label" for="showActiveOnly">Solo activos</label>
                  </div>
          ...
          <div *ngIf="!loading && filteredLoans.length === 0" class="alert alert-info d-flex align-items-center">
            <i class="bi bi-info-circle me-2 fs-4"></i>
            <div>
              <strong>No se encontraron resultados.</strong>
              <p class="mb-0">No hay préstamos disponibles en el registro con los filtros seleccionados.</p>
              <button class="btn btn-sm btn-link ps-0 pt-1" (click)="resetFilters(searchInput)">Mostrar todos los préstamos</button>
            </div>
          </div>

          <!-- Vista de tabla para pantallas grandes -->  
          <div *ngIf="!loading && filteredLoans.length > 0" class="card shadow-sm d-none d-md-block">
            <div class="card-body p-0">
              <div class="table-responsive">
                <table class="table table-hover align-middle mb-0">
                  ...
                  <tbody>
                    <tr *ngFor="let loan of filteredLoans; let i = index" [ngClass]="{'bg-light': i % 2 === 0}">
                      <td class="ps-3">
                        <div class="d-flex align-items-center">
                          <div class="loan-icon rounded-circle bg-light text-primary me-2 d-flex align-items-center justify-content-center">
                            <i class="bi bi-book"></i>
                          </div>
                          <div>{{ loan.libro.titulo }}</div>
                        </div>
                      </td>
                      <td>{{ loan.usuario.nombre }} {{ loan.usuario.apellidos }}</td>
                      <td>{{ loan.fechaPrestamo | date: 'dd/MM/yyyy' }}</td>
                      <td>{{ loan.fechaDevolucion | date: 'dd/MM/yyyy' }}</td>
                      <td class="text-center">
                        <span class="badge" [ngClass]="loan.vencido ? 'bg-danger' : 'bg-success'">
                          {{ loan.vencido ? 'Sí' : 'No' }}
                        </span>
                      </td>
                      <td class="text-center">
                        <span class="badge" [ngClass]="getStatusBadgeClass(loan.estado)">
                          {{ loan.estado }}
                        </span>
                      </td>
                      <td class="text-center pe-3">
                        <div class="btn-group">
                          <button class="btn btn-sm btn-outline-primary" (click)="openForm(loan)" title="Editar préstamo" 
                                  [disabled]="loan.estado !== 'ACTIVO'">
                            <i class="bi bi-pencil"></i>
                          </button>
                          <button class="btn btn-sm btn-outline-success" (click)="returnBook(loan)" title="Registrar devolución" 
                                  [disabled]="loan.estado !== 'ACTIVO'">
                            <i class="bi bi-check-circle"></i>
                          </button>
                          <button class="btn btn-sm btn-outline-danger" (click)="cancelLoan(loan)" title="Cancelar préstamo" 
                                  [disabled]="loan.estado !== 'ACTIVO'">
            ...
            <div class="card-footer bg-white py-2">
              <div class="d-flex justify-content-between align-items-center">
                <span class="text-muted small">Mostrando {{ filteredLoans.length }} de {{ loans.length }} préstamos</span>
              </div>
            </div>
          </div>
          ...
          <div *ngIf="!loading && filteredLoans.length > 0" class="d-md-none">
            <div class="d-flex justify-content-between align-items-center mb-3">
              ...
              <span class="badge bg-primary rounded-pill">{{ filteredLoans.length }} préstamos</span>
            </div>
                  ...
                  <div class="card-body p-3">
                    <div class="d-flex justify-content-between align-items-start mb-2">
                      <div class="d-flex align-items-center">
                        ...
                        <h6 class="card-title mb-0 text-primary">{{ loan.libro.titulo }}</h6>
                      </div>
                      <span class="badge" [ngClass]="getStatusBadgeClass(loan.estado)">
                        {{ loan.estado }}
                      </span>
                    ...
                    <div class="row mb-2">
                      <div class="col-12">
                        ...
                        <span>{{ loan.usuario.nombre }} {{ loan.usuario.apellidos }}</span>
                      </div>
                    </div>
                    ...
                    <div class="row mb-2">
                      <div class="col-6">
                        ...
                        <span>{{ loan.fechaPrestamo | date: 'dd/MM/yyyy' }}</span>
                      </div>
                      <div class="col-6">
                        <small class="text-muted d-block">Fecha devolución</small>
                        <span>{{ loan.fechaDevolucion | date: 'dd/MM/yyyy' }}</span>
                      </div>
                    </div>
          ...
          <div class="modal fade" [ngClass]="{ 'show d-block': showReturnConfirm }" tabindex="-1" role="dialog">
            <div class="modal-dialog" role="document">
              <div class="modal-content">
                <div class="modal-header">
                  <h5 class="modal-title">Confirmar devolución</h5>
                  <button type="button" class="btn-close" (click)="closeReturnConfirm()"></button>
                </div>
                <div class="modal-body">
                  <p>¿Confirmar la devolución del siguiente libro?</p>
                  <div class="card bg-light mb-3">
                    <div class="card-body">
                      <p class="mb-1"><strong>Libro:</strong> {{ selectedLoan?.libro?.titulo }}</p>
                      <p class="mb-1"><strong>Estudiante:</strong> {{ selectedLoan?.usuario?.nombre }} {{ selectedLoan?.usuario?.apellidos }}</p>
                      <p class="mb-0"><strong>Fecha de préstamo:</strong> {{ selectedLoan?.fechaPrestamo | date: 'dd/MM/yyyy' }}</p>
                    </div>
                  </div>
                </div>
                <div class="modal-footer">
                  <button type="button" class="btn btn-outline-secondary" (click)="closeReturnConfirm()">Cancelar</button>
          ...
          <div class="modal fade" [ngClass]="{ 'show d-block': showCancelConfirm }" tabindex="-1" role="dialog">
            <div class="modal-dialog" role="document">
              <div class="modal-content">
                <div class="modal-header">
                ...
                <div class="modal-footer">
                  <button type="button" class="btn btn-outline-secondary" (click)="closeCancelConfirm()">Cancelar</button>
...
export class LoanListComponent implements OnInit {
  ...
  loadLoans(): void {
    ...
    this.loanService.getAllLoans().subscribe({
      ...
      error: (err) => {
        ...
        this.loading = false;
        this.alertService.error(err.message || 'Error al cargar los préstamos. Por favor, intente nuevamente más tarde.');
      }
    });
  }
  ...
  searchLoans(term: string): void {
    ...
    if (!this.searchTerm) {
      // Si no hay término de búsqueda, carga todos los préstamos
      this.loadLoans();
      return;
    }
    ...
    this.loanService.getLoansByStudenOrTitle(this.searchTerm).subscribe({
      ...
      error: (err) => {
        ...
        if (err.message && err.message.includes('404')) {
          this.loans = [];
          this.filteredLoans = [];
          this.alertService.info(`No se encontraron préstamos que coincidan con "${this.searchTerm}"`);
        } else {
          this.alertService.error(err.message || 'Error al buscar préstamos. Por favor, intente nuevamente.');
        }
      }
    });
  }
  getStatusBadgeClass(status: string): string {
    switch(status) {
      ...
      case 'VENCIDO': return 'bg-warning text-dark';
    ...
    if (loan.id) {
      this.loanService.updateLoan(loan.id, loan).subscribe({
        ...
        error: (err) => {
          ...
          this.alertService.error(err.message || 'Error al actualizar préstamo. Por favor, intente nuevamente.');
        }
      });
    } else {
      this.loanService.createLoan(loan).subscribe({
        ...
        error: (err) => {
          ...
          this.alertService.error(err.message || 'Error al registrar préstamo. Por favor, verifique los datos e intente nuevamente.');
        }
      });
    }
  }
  ...
  confirmReturn(): void {
    ...
    this.loanService.returnBook(this.selectedLoan.id).subscribe({
      ...
      error: (err) => {
        ...
        this.closeReturnConfirm();
        this.alertService.error(err.message || 'Error al registrar devolución. Por favor, intente nuevamente.');
      }
    });
  }
  ...
  confirmCancel(): void {
    ...
    this.loanService.cancelLoan(this.selectedLoan.id).subscribe({
      ...
      error: (err) => {
        ...
        this.closeCancelConfirm();
        this.alertService.error(err.message || 'Error al cancelar préstamo. Por favor, intente nuevamente.');
      }
    });
  }
